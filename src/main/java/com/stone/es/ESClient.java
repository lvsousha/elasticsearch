...
//                					.put("client.transport.nodes_sampler_interval",  "1000")	//定时对列出和连接的节点进行抽样/ ping操作。 默认为5秒。
	...
	public static Client createClientBySetting(String clusterName, String nodeName, String url) throws Exception{
		Settings settings  =  Settings.settingsBuilder()
                					.put("cluster.name",  clusterName)
                					.put("node.name",  nodeName)
                					.build();
	...
	public static Client createClientShield(String clusterName, String shieldUser, String url) throws NumberFormatException, UnknownHostException{
		Settings settings  =  Settings.settingsBuilder()
                					.put("cluster.name",  clusterName)
                					.put("shield.user", shieldUser)
                					.build();
