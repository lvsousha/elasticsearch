...
public class ESIndexClientBasic implements ESIndexClient{
	...
	public Boolean createIndex(Client client, String index, String type, List<Mapping> mappings) throws Exception {
		...
		CreateIndexResponse response = indicesAdminClient
											.prepareCreate(index) 
					        				.addMapping(type, buildMappings(type, mappings))
					        				.get();
...
//		log.info(csr.getState().toString());
		...
		for(ObjectCursor<String> key : iom.keys()){
			...
			model.setSettings(indexMetaData.getSettings().getAsMap());
			list.add(model);
		}
	...
	private List<AliasMetaData> getAliases(Iterator<ObjectObjectCursor<String, AliasMetaData>> iooca){
	...
	private List<MappingMetaData> getMappings(Iterator<ObjectObjectCursor<String, MappingMetaData>> ioocm) throws Exception{
	...
	public Boolean appendMapping(Client client, String index, String type, List<Mapping> append) throws Exception {
		...
		PutMappingResponse response = indicesAdminClient.preparePutMapping(index)
				.setType(type)
				.setSource(buildMappings(type, append))
				.get();
